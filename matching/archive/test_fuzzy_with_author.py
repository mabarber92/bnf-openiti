"""
Greedy search test with author matching.

Separates title and author matching signals to understand which is driving results.
Produces diagnostic output showing:
- Title-only matches
- Author-only matches
- Author+Title combined matches
"""
...
def build_openiti_author_candidates(author_uri: str) -> dict[str, list[str]]:
    """Extract author name candidates from OpenITI author record."""
    candidates = {"lat": [], "ara": []}
    ...
    # Transliterated name (from slug)
    if author.get("name_slug"):
        candidates["lat"].append(author["name_slug"])

    # Arabic names (multiple components)
    for field in ["name_shuhra_ar", "name_ism_ar", "name_kunya_ar", "name_laqab_ar", "name_nisba_ar"]:
        if author.get(field):
...
def run_searches(thresholds: list[float] = None) -> list[SearchResult]:
    ...
    print(f"\nSearching {len(test_pairs)} BNF records (with author and title matching)...")
...
def write_results(results: list[SearchResult], output_dir: Path = Path("matching")) -> None:
    """Write results and generate reports."""
    output_dir.mkdir(exist_ok=True)

    # One report for each signal
    for signal in ["title", "author", "combined"]:
        report_path = output_dir / f"signal_diagnosis_{signal}.txt"

        with open(report_path, "w", encoding="utf-8") as f:
            f.write(f"SIGNAL DIAGNOSIS: {signal.upper()}\n")
            f.write("="*80 + "\n\n")
            ...
            for threshold in thresholds:
                ...
                if signal == "title":
                    all_matches = sum(len(r.matched_uris_by_title) for r in threshold_results)
                elif signal == "author":
                    all_matches = sum(len(r.matched_uris_by_author) for r in threshold_results)
                else:
                    all_matches = sum(len(r.matched_uris_combined) for r in threshold_results)
    ...
    print(f"Diagnostic reports written to {output_dir}/signal_diagnosis_*.txt")
