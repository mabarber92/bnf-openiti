"""
Greedy search test: realistic matching performance and precision/recall.

For each BNF record in the correspondence set, searches against ALL OpenITI
books and tracks:
- Correct matches found (recall)
- False positive matches (precision issues)
- Execution time per threshold

This tests what the real matcher will face: searching one BNF record against
9500+ OpenITI books and finding the right one without false positives.

Usage:
    python matching/test_fuzzy_greedy_search.py

Output:
    matching/fuzzy_greedy_results.csv — all matches found
    matching/fuzzy_greedy_report.txt — precision/recall/timing summary
"""
...
def search_bnf_against_all_openiti(
    ...
    Returns which URIs match above threshold.
    """
...
def run_searches(thresholds: list[float] = None) -> list[SearchResult]:
    ...
    print(f"\nSearching {len(test_pairs)} BNF records against {len(openiti_books)} OpenITI books...")
...
def write_results(results: list[SearchResult], output_dir: Path = Path("matching")) -> None:
    ...
    csv_path = output_dir / "fuzzy_greedy_results.csv"
    report_path = output_dir / "fuzzy_greedy_report.txt"
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("GREEDY SEARCH TEST REPORT\n")
        f.write("="*80 + "\n\n")
        ...
        if best_metrics["precision"] >= 0.95 and best_metrics["recall"] >= 0.95:
            ...
            f.write("  Consider transliteration normalization or different approach.\n")
